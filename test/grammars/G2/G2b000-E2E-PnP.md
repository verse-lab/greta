## G2b E2E Illustration 

### Step 1: Extract CFG $G$

* V = { ident, decl, const, exp, exp2, stmt, stmts }
...
* P = {\
    ...
    exp2 $\to$ exp2 DASH exp2 \
    exp2 $\to$ exp2 STAR exp2 \
    exp2 $\to$ ident \
    exp2 $\to$ const \
    exp2 $\to$ LPAREN exp RPAREN 
...
* $Q_{g}$ = { ident, decl, const, exp, exp2, stmt, stmts, $\epsilon$ }
...
* $\Delta_{g}$ = {\
    ...
    exp $\to_{(\texttt{PLUS}, 2)}$ exp PLUS exp2 \
    exp $\to_{(\varepsilon, 1)}$ exp2

    exp2 $\to_{(\texttt{DASH}, 2)}$ exp2 DASH exp2 \
    exp2 $\to_{(\texttt{STAR}, 2)}$ exp2 STAR exp2 \
    exp2 $\to_{(\varepsilon, 1)}$ ident \
    exp2 $\to_{(\varepsilon, 1)}$ const \
    exp2 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN exp RPAREN 
...
* Traverse $\Delta$ to get $L_{Q}$: 
  { (stmts, 0), (stmt, 1), (decl, 2), (ident, 2), (exp, 2), (exp2, 3), (const, 4) }

* Based on $\Delta$ and $L_{Q}$, get $O_{bp}$: 
  { <($\varepsilon$, 1), 0>,  <($\varepsilon$, 2), 0>,  <(SEMI, 1), 1>,  <(EQ, 3), 1>,  <(IF, 4), 1>,  <(IF, 6), 1>,  <(RET, 2), 1>,  <(WHILE, 4), 1>,  <(LBRACE, 2), 1>,  <(TINT, 3), 2>,  <(PLUS, 2), 2>,  <($\varepsilon$, 1), 2>,  <(DASH, 2), 3>,  <(STAR, 2), 3>,  <($\varepsilon$, 1), 3>,  <(LPARENRPAREN, 1), 3> }

That is, $O_{bp}$ is order $\to$ symbols mapping
- 0 $\to$ [ ($\varepsilon$, 1), ($\varepsilon$, 2) ]
- 1 $\to$ [ (SEMI, 1), (EQ, 3), (IF, 4), (IF, 6), (RET, 2), (WHILE, 4), (LBRACE, 2) ]
- 2 $\to$ [ (TINT, 3), (PLUS, 2), ($\varepsilon$, 1) ]
- 3 $\to$ [ (DASH, 2), (STAR, 2), ($\varepsilon$, 1), (LPARENRPAREN, 1) ]
...
- 1 $\to$ [ (SEMI, 1), (EQ, 3), (IF, 6), (RET, 2), (WHILE, 4), (LBRACE, 2), (DASH, 2) ]
- 2 $\to$ [ (TINT, 3), (PLUS, 2), (STAR, 2), ($\varepsilon$, 1), (LPARENRPAREN, 1) ]

* $O_{a}$: [ <(STAR, 2), "l"> ]

### Step 6: Learn $A_{r}$

* $Q_{r}$ = { e1, e2, e3, e4, ident, const, $\epsilon$ }
* $F$ = { (INT, 0), (IDENT, 0), (IF, 4), (IF, 6), (TINT, 3), (RET, 2), (WHILE, 4), (SEMI, 1), (LBRACE, 2), (PLUS, 2), (DASH, 2), (STAR, 2), (EQ, 3), (LPARENRPAREN, 1) }
* $I_{r}$ = { stmts }
* $\Delta_{r}$ = {\
    ...
    e2 $\to_{(\texttt{DASH}, 2)}$ e2 DASH e2 \
    _e2 $\to_{(\varepsilon, 1)}$ e3_

    e3 $\to_{(\texttt{TINT}, 3)}$ TINT ident EQ e3 \
    e3 $\to_{(\texttt{PLUS}, 2)}$ e3 PLUS e3 \
    e3 $\to_{(\texttt{STAR}, 2)}$ e4 STAR e3 \
    e3 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN e3 RPAREN \
    _e3 $\to_{(\varepsilon, 1)}$ e4_
    
    e4 $\to_{(\varepsilon, 1)}$ ident \
    e4 $\to_{(\varepsilon, 1)}$ const \
    _e4 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN e1 RPAREN_
...
* $Q$ = { stmts_e1, stmt_e1, decl_e2, exp_e2, exp_e1, stmt_e2, stmts_e2, exp_e3, exp2_e3, exp2_e2, exp2_e4, exp_e4, ident_ident, const_const }
* $I$ = { stmts_e1 }
* $\Delta$ = {\
    ...
    **exp_e2** $\to_{(\texttt{PLUS}, 2)}$ exp_e3 PLUS **exp2_e3** \
    exp_e2 $\to_{(\texttt{DASH}, 2)}$ **exp2_e2** DASH exp2_e2 \
    exp_e2 $\to_{(\texttt{STAR}, 2)}$ **exp2_e4** STAR exp2_e3 \
    ...
    **exp_e1** $\to_{(\texttt{PLUS}, 2)}$ exp_e3 PLUS exp2_e3 \
    exp_e1 $\to_{(\texttt{DASH}, 2)}$ exp2_e2 DASH exp2_e2 \
    exp_e1 $\to_{(\texttt{STAR}, 2)}$ exp2_e4 STAR exp2_e3 \
    ...
    **exp_e3** $\to_{(\texttt{PLUS}, 2)}$ exp_e3 PLUS exp2_e3 \
    exp_e3 $\to_{(\texttt{STAR}, 2)}$ **exp_e4** STAR exp_e3 \
    ...
    **exp2_e3** $\to_{(\texttt{STAR}, 2)}$ exp2_e4 STAR exp2_e3 \
    exp2_e3 $\to_{(\varepsilon, 1)}$ ident_ident \
    exp2_e3 $\to_{(\varepsilon, 1)}$ const_const \
    exp2_e3 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN exp_e3 RPAREN \
    exp2_e3 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN exp_e1 RPAREN 

    **exp2_e2** $\to_{(\texttt{DASH}, 2)}$ exp2_e2 DASH exp2_e2 \
    exp2_e2 $\to_{(\texttt{STAR}, 2)}$ exp2_e4 STAR exp2_e3 \
    exp2_e2 $\to_{(\varepsilon, 1)}$ ident_ident \
    exp2_e2 $\to_{(\varepsilon, 1)}$ const_const \
    exp2_e2 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN exp_e3 RPAREN \
    exp2_e2 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN exp_e1 RPAREN

    **exp2_e4** $\to_{(\varepsilon, 1)}$ ident_ident \
    exp2_e4 $\to_{(\varepsilon, 1)}$ const_const \
    exp2_e4 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN exp_e1 RPAREN 
    
    **exp_e4** $\to_{(\varepsilon, 1)}$ ident_ident \
    exp_e4 $\to_{(\varepsilon, 1)}$ const_const \
    exp_e4 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN exp_e1 RPAREN 
...
* { (exp_e2, exp_e1), (exp2_e4, exp_e4) }

### Step 9: Remove duplicate states and rename states

* $Q$ = { stmts_e1, stmt_e1, decl_e2, exp_e2, stmt_e2, stmts_e2, exp_e3, exp2_e3, exp2_e2, exp2_e4, ident_ident, const_const }
  - Rename maps:
    + stmts_e1 $\to$ e1
    + stmt_e1 $\to$ x1
    + decl_e2 $\to$ x2
...
* $\Delta$ = {\
    ...
    x1 $\to_{(\texttt{IF}, 6)}$ IF LPAREN x3 RPAREN x4 ELSE x4 \
    x1 $\to_{(\texttt{RET}, 2)}$ RET x3 SEMI \
    x1 $\to_{(\texttt{WHILE}, 4)}$ WHILE LPAREN x3 RPAREN x4 \
    ...
    x3 $\to_{(\texttt{STAR}, 2)}$ x9 STAR x7 \
    x3 $\to_{(\varepsilon, 1)}$ ident_ident \
    x3 $\to_{(\varepsilon, 1)}$ const_const \
    x3 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN x6 RPAREN \
    ...
    x4 $\to_{(\texttt{LBRACE}, 2)}$ LBRACE x5 RBRACE

    x5 $\to_{(\varepsilon, 1)}$ $\epsilon$

    x6 $\to_{(\texttt{PLUS}, 2)}$ x6 PLUS x7 \
    x6 $\to_{(\texttt{STAR}, 2)}$ x9 STAR x6 \
    x6 $\to_{(\varepsilon, 1)}$ ident \
    x6 $\to_{(\varepsilon, 1)}$ const \
    x6 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN x6 RPAREN \
    x6 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN x3 RPAREN

    x7 $\to_{(\texttt{STAR}, 2)}$ x9 STAR x7 \
    x7 $\to_{(\varepsilon, 1)}$ ident \
    x7 $\to_{(\varepsilon, 1)}$ const \
    x7 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN x6 RPAREN \
    x7 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN x3 RPAREN 

    x8 $\to_{(\texttt{DASH}, 2)}$ x8 DASH x8 \
    x8 $\to_{(\texttt{STAR}, 2)}$ x9 STAR x7 \
    x8 $\to_{(\varepsilon, 1)}$ ident \
    x8 $\to_{(\varepsilon, 1)}$ const \
    x8 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN x6 RPAREN \
    x8 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN x3 RPAREN

    x9 $\to_{(\varepsilon, 1)}$ ident \
    x9 $\to_{(\varepsilon, 1)}$ const \
    x9 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN x3 RPAREN 
...
* $Q$ = { e1, x1, x2, x3, x4, x5, x6, x7, x8, x9, ident, const, $\epsilon$ }
...
* $\Delta$ = {\
    ...
    x6 $\to_{(\texttt{PLUS}, 2)}$ x6 PLUS x7 \
    x6 $\to_{(\varepsilon, 1)}$ x7

    x8 $\to_{(\texttt{DASH}, 2)}$ x8 DASH x8 \
    x8 $\to_{(\varepsilon, 1)}$ x7

    x7 $\to_{(\texttt{STAR}, 2)}$ x9 STAR x7 \
    x7 $\to_{(\varepsilon, 1)}$ x9
    x7 $\to_{(\texttt{LPARENRPAREN}, 1)}$ LPAREN x6 RPAREN 
