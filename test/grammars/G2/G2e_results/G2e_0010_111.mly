...
x8:
  ...
  | e1=x7 PLUS e2=x9  { loc $startpos $endpos @@ Bop(Add, e1, e2) }
  ;
...
x5:
  | e1=x8 STAR e2=x3  { loc $startpos $endpos @@ Bop(Mul, e1, e2) }
  | id=x7            { loc $startpos $endpos @@ Id (id) }
  | e1=x5 DASH e2=x3  { loc $startpos $endpos @@ Bop(Sub, e1, e2) }
  | id=x12            { loc $startpos $endpos @@ Id (id) }
  | LPAREN e=x4 RPAREN { e }
  ;
