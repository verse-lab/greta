...
e1: 
  ...
  | p=x7 EQ e=x4 SEMI { loc $startpos $endpos @@ Assn(p,e) }
  | e=x4 LPAREN COMMA es=x4 RPAREN SEMI { loc $startpos $endpos @@ SCall (e, es) }
  | IF e=bool_expr THEN s1=e1 ELSE s2=e1  { loc $startpos $endpos @@ If(e,[s1],[s2]) }
  | RETURN SEMI         { loc $startpos $endpos @@ Ret(None) }
  | RETURN e=x4 SEMI   { loc $startpos $endpos @@ Ret(Some e) }
  | WHILE LPAREN e=x4 RPAREN b=e1  { loc $startpos $endpos @@ While(e, b) } 
  | NULL  { loc $startpos $endpos @@ CNull }

x5:
  | x6 { $1 }
  | e1=x5 EQEQ e2=x5 { loc $startpos $endpos @@ Bop (Eq, e1, e2) }
  ;
...
x2:
  | x3 { $1 }
  | e1=x3 PLUS e2=x2 { loc $startpos $endpos @@ Bop (Add, e1, e2) }
  ;
