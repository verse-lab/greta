...
x8:  
  | x5 LBRACKET x5 RBRACKET { loc $startpos $endpos @@ Index ($1, $3) }

x2: 
  | d=x9 SEMI        { loc $startpos $endpos @@ Decl(d) }
  | p=x8 EQ e=x3 SEMI { loc $startpos $endpos @@ Assn(p,e) }
...
x7:
  | BANG x7         { loc $startpos $endpos @@ Uop (Lognot, $2) }
  | TILDE x7        { loc $startpos $endpos @@ Uop (Bitnot, $2) }
...
x6:
  | x7 { $1 }
  | DASH x6         { loc $startpos $endpos @@ Uop (Neg, $2) }
  ;

x5:
  | x6 { $1 }
  | x5 EQEQ x7 { loc $startpos $endpos @@ Bop (Eq, $1, $3) }
  ;
...
x3:
  | x4 { $1 }
  | x3 STAR x7 { loc $startpos $endpos @@ Bop (Mul, $1, $3) }
  ;
