...
test21:
  | test36  { $1 }
  | test21 AND test36  { Ast.(BoolOp { op = And; values = [$1; $3] }) }
  ;
...
test5:
  | test7  { $1 }
  | condition=test7 QUESTION ifTrue=test5 COLON ifFalse=test5  { Ast.(IfExp {condition; ifTrue; ifFalse}) }
  ;

test7:
  | test21 OR test7  { Ast.(BoolOp { op = Or; values = [$1; $3] }) }
  | test21  { $1 }
  ;
