...
let create_log_filename dir =
  if (not (Stdlib.Sys.file_exists dir)) || not (Stdlib.Sys.is_directory dir) then
...
module StdlibTracker = struct
  ...
  let get_stdlib_dirs () =
    let env_dirs =
      Option.value_map
        (Stdlib.Sys.getenv_opt scilla_stdlib_env)
  ...
  let find_lib_dir name =
    ...
    List.find dirs ~f:(fun d ->
        Stdlib.Sys.file_exists (d ^/ name ^. file_extn_library))

  (* adt.tname -> defining library. *)
  (* let adt_deflib_dict = Stdlib.Hashtbl.create 5

  let add_deflib_adttyp tname libname =
    Stdlib.Hashtbl.add adt_deflib_dict tname libname

  let lookup_deflib_adttyp tname = Stdlib.Hashtbl.find_opt adt_deflib_dict tname

  (* Reset internal state. *)
  let reset () =
    stdlib_dirs := [];
    Stdlib.Hashtbl.reset adt_deflib_dict *)
end
