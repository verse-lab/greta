...
| simple_exp { $1 }
| NOT exp { Not($2) }
| MINUS exp { match $2 with | Float(f) -> Float(-.f) | e -> Neg(e) }
...
| exp SEMICOLON exp { Let((Id.gentmp Type.Unit, TypeUnit), $1, $3) }
| ARRAY_CREATE simple_exp simple_exp { Array($2, $3) }
| error { failwith (Printf.sprintf "parse error near characters %d-%d" (Parsing.symbol_start ()) (Parsing.symbol_end ())) }
