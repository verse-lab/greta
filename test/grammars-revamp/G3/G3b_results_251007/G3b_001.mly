...
ex6:
  | e1=ex6 PLUS e2=exp22  { loc $startpos $endpos @@ Bop(Add, e1, e2) }
  | ex7  { $1 }
  ;
...
exp22:
  | e1=exp24 STAR e2=exp22  { loc $startpos $endpos @@ Bop(Mul, e1, e2) }
  | exp24  { $1 }
  ;
...
stmt1:
  | stmt3  { $1 }
  | IF LPAREN e=ex6 RPAREN s1=stmt1  { loc $startpos $endpos @@ If(e, [s1], []) }
  ;

stmt3:
  | d=decl10 SEMI  { loc $startpos $endpos @@ Decl(d) }
  | IF LPAREN e=ex6 RPAREN s1=stmt3 ELSE s2=stmt3  { loc $startpos $endpos @@ If(e, [s1], [s2]) }
  | stmt5  { $1 }
