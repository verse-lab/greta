...
exp25:
  | exp27  { $1 }
  | e1=exp23 STAR e2=exp27  {  loc $startpos $endpos @@ Bop(Mul, e1, e2)  }
  ;
...
stmt2:
  | IF LPAREN e=ex12 RPAREN s1=stmt1  {  loc $startpos $endpos @@ If(e, [s1], [])  }
  | stmt6  { $1 }
  ;

stmt6:
  | stmt10  {  $1  }
  | IF LPAREN e=ex11 RPAREN s1=stmt6 ELSE s2=stmt6  {  loc $startpos $endpos @@ If(e, [s1], [s2])  }
  | d=decl19 SEMI  {  loc $startpos $endpos @@ Decl(d)  }
  | id=iden20 EQ e=ex12 SEMI  {  loc $startpos $endpos @@ Assn(id, e)  }
  ;
