...
ex1:
  | e1=ex3 PLUS e2=ex1  {  loc $startpos $endpos @@ Bop(Add, e1, e2)  }
  | ex3  { $1 }
  ;

ex3:
  | e1=ex3 DASH e2=ex3  {  loc $startpos $endpos @@ Bop(Sub, e1, e2)  }
  | ex5  { $1 }
  ;

ex5:
  | e1=ex8 STAR e2=ex5  {  loc $startpos $endpos @@ Bop(Mul, e1, e2)  }
  | ex8  { $1 }
  ;
...
stmt2:
  | IF LPAREN e=ex1 RPAREN s1=stmt2 ELSE s2=stmt2  {  loc $startpos $endpos @@ If(e, [s1], [s2])  }
  | stmt4  { $1 }
  ;

stmt4:
  | stmt7  { $1 }
  | IF LPAREN e=ex1 RPAREN s1=stmt4  {  loc $startpos $endpos @@ If(e, [s1], [])  }
  ;
