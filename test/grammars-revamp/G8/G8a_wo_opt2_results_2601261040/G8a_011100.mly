...
test34:
  | test50  { $1 }
  | condition=test34 QUESTION ifTrue=test34 COLON ifFalse=test50  {  Ast.(IfExp {condition; ifTrue; ifFalse})  }
  ;
...
test7:
  | test6 OR test9  {  Ast.(BoolOp { op = Or; values = [$1; $3] })  }
  | test9  { $1 }
  ;

test9:
  | test34  { $1 }
  | test9 AND test34  {  Ast.(BoolOp { op = And; values = [$1; $3] })  }
  ;
