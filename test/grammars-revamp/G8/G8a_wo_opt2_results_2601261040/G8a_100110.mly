...
test34:
  | test50  { $1 }
  | test50 AND test34  {  Ast.(BoolOp { op = And; values = [$1; $3] })  }
  ;
...
test7:
  | condition=test9 QUESTION ifTrue=test6 COLON ifFalse=test6  {  Ast.(IfExp {condition; ifTrue; ifFalse})  }
  | test9  { $1 }
  ;

test9:
  | test9 OR test34  {  Ast.(BoolOp { op = Or; values = [$1; $3] })  }
  | test34  { $1 }
  ;
