...
test35:
  | condition=test35 QUESTION ifTrue=test35 COLON ifFalse=test50  {  Ast.(IfExp {condition; ifTrue; ifFalse})  }
  | test50  { $1 }
  ;
...
test7:
  | test9 AND test6  {  Ast.(BoolOp { op = And; values = [$1; $3] })  }
  | test9  { $1 }
  ;

test9:
  | test9 OR test35  {  Ast.(BoolOp { op = Or; values = [$1; $3] })  }
  | test35  { $1 }
  ;
