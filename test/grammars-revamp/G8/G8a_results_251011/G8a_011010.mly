...
test22:
  | test36  { $1 }
  | condition=test22 QUESTION ifTrue=test22 COLON ifFalse=test36  { Ast.(IfExp {condition; ifTrue; ifFalse}) }
  ;
...
test5:
  | test7  { $1 }
  | test7 AND test5  { Ast.(BoolOp { op = And; values = [$1; $3] }) }
  ;

test7:
  | test7 OR test22  { Ast.(BoolOp { op = Or; values = [$1; $3] }) }
  | test22  { $1 }
  ;
