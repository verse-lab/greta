...
test4:
  | test6 AND test4  {  Ast.(BoolOp { op = And; values = [$1; $3] })  }
  | test6  { $1 }
  ;

test6:
  | test8  { $1 }
  | condition=test6 QUESTION ifTrue=test6 COLON ifFalse=test8  {  Ast.(IfExp {condition; ifTrue; ifFalse})  }
  ;

test8:
  | test8 OR test25  {  Ast.(BoolOp { op = Or; values = [$1; $3] })  }
  | test25  { $1 }
  ;
