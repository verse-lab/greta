...
test4:
  | test4 OR test6  {  Ast.(BoolOp { op = Or; values = [$1; $3] })  }
  | test6  { $1 }
  ;

test6:
  | test8  { $1 }
  | condition=test8 QUESTION ifTrue=test6 COLON ifFalse=test6  {  Ast.(IfExp {condition; ifTrue; ifFalse})  }
  ;

test8:
  | test8 AND test25  {  Ast.(BoolOp { op = And; values = [$1; $3] })  }
  | test25  { $1 }
  ;
