...
not_22:
  | comp20  {  $1  }
  | NOT not_22  {  Ast.(UnaryOp { op = Ast.Not; operand = $2 })  }
  ;
...
test21:
  | test36  { $1 }
  | condition=test21 QUESTION ifTrue=test21 COLON ifFalse=test36  {  Ast.(IfExp {condition; ifTrue; ifFalse})  }
  ;
...
test5:
  | test7  { $1 }
  | test5 OR test7  {  Ast.(BoolOp { op = Or; values = [$1; $3] })  }
  ;

test7:
  | test21 AND test7  {  Ast.(BoolOp { op = And; values = [$1; $3] })  }
  | test21  { $1 }
  ;
