...
exp13:
  | e1=exp14 STAR e2=exp13  { loc $startpos $endpos @@ Bop (Mul, e1, e2) }
  | exp14  { $1 }
  ;

exp14:
  | e1=exp14 DASH e2=exp15  { loc $startpos $endpos @@ Bop (Sub, e1, e2) }
  | exp15  { $1 }
  ;

exp15:
  | exp16  { $1 }
  | e1=exp16 EQEQ e2=exp15  { loc $startpos $endpos @@ Bop (Eq, e1, e2) }
  ;

exp16:
  | e1=exp16 PLUS e2=exp22  { loc $startpos $endpos @@ Bop (Add, e1, e2) }
  | exp22  { $1 }
  ;
...
expr1:
  ...
  | d=vdec10 SEMI  { loc $startpos $endpos @@ Decl(d) }
  | RETURN SEMI  { loc $startpos $endpos @@ Ret(None) }
  | NULL  { loc $startpos $endpos @@ CNull }
  | RETURN e=exp13 SEMI  { loc $startpos $endpos @@ Ret(Some e) }
  | p=lh9 EQ e=exp13 SEMI  { loc $startpos $endpos @@ Assn(p,e) }
  ;
...
vdec10:
  | VAR id=iden8 EQ init=exp13  { loc $startpos $endpos @@ {id; init} }
  ;
