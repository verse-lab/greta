...
exp110:
  | e1=exp110 STAR e2=exp112  {  loc $startpos $endpos @@ Bop (Mul, e1, e2)  }
  | exp112  { $1 }
  ;
...
exp17:
  | exp18  { $1 }
  | e1=exp18 DASH e2=exp16  {  loc $startpos $endpos @@ Bop (Sub, e1, e2)  }
  ;

exp18:
  | e1=exp18 PLUS e2=exp25  {  loc $startpos $endpos @@ Bop (Add, e1, e2)  }
  | exp19  { $1 }
  ;

exp19:
  | e1=exp19 EQEQ e2=exp110  {  loc $startpos $endpos @@ Bop (Eq, e1, e2)  }
  | exp110  { $1 }
  ;
...
expr2:
  ...
  | p=lh14 EQ e=exp17 SEMI  {  loc $startpos $endpos @@ Assn(p,e)  }
  | NULL  {  loc $startpos $endpos @@ CNull  }
  | d=vdec15 SEMI  {  loc $startpos $endpos @@ Decl(d)  }
...
vdec15:
  | VAR id=iden13 EQ init=exp16  {  loc $startpos $endpos @@ {id; init}  }
  ;
