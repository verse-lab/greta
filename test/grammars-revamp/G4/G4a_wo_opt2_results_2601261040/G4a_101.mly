...
exp211:
  | exp214  { $1 }
  | e1=exp214 STAR e2=exp210  {  loc $startpos $endpos @@ Bop (Mul, e1, e2)  }
  ;
...
exp39:
  | e1=exp39 EQEQ e2=exp313  {  loc $startpos $endpos @@ Bop (Eq, e1, e2)  }
  | exp313  { $1 }
  ;
...
expr2:
  ...
  | d=vdec17 SEMI  {  loc $startpos $endpos @@ Decl(d)  }
  | RETURN SEMI  {  loc $startpos $endpos @@ Ret(None)  }
  | p=lh16 EQ e=exp18 SEMI  {  loc $startpos $endpos @@ Assn(p,e)  }
...
vdec17:
  | VAR id=iden15 EQ init=exp17  {  loc $startpos $endpos @@ {id; init}  }
  ;
